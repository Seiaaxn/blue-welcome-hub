...
export const SectionTitle = ({ title, onViewMore }: { title: string; onViewMore?: () => void }) => (
  <div className="flex items-center justify-between mb-3 sm:mb-4">
    <h3 className="text-xl sm:text-3xl font-black flex items-center gap-2 sm:gap-3">
      <span className="h-6 sm:h-7 w-1.5 rounded-full bg-primary" />
      {title}
    </h3>
    {onViewMore && (
      <button onClick={onViewMore} className="text-xs sm:text-sm text-primary hover:underline flex items-center gap-1 font-bold">
...
export const PortraitCard = ({ a, onClick, rank }: { a: AnimeCard; onClick: (a: AnimeCard) => void; rank?: number }) => (
  <button onClick={() => onClick(a)} className="group text-left relative">
    <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-secondary border border-border">
      ...
      {rank != null && (
        <span className="absolute left-1 top-1 text-2xl sm:text-3xl font-black text-foreground/90">{rank}</span>
      )}
      ...
      {a.episodes > 0 && (
        <span className="absolute left-1.5 bottom-1.5 px-1.5 py-0.5 rounded bg-primary/90 text-[10px] font-black text-primary-foreground">EP {a.episodes}</span>
      )}
    ...
    <p className="text-xs sm:text-sm font-semibold mt-1.5 line-clamp-2 group-hover:text-primary transition">{a.title}</p>
    <p className="text-[10px] text-muted-foreground mt-0.5">{a.type}{a.episodes > 0 ? ` • ${a.episodes} Eps` : ""}</p>
  </button>
);
